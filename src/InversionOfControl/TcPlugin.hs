...
lookupModule mod_nm _pkg = do
  ...
  found_module ← tcPluginIO $ do
    Finder.findPluginModule
      (hsc_FC hsc_env)
      (initFinderOpts $ hsc_dflags hsc_env)
      (hsc_units hsc_env)
      (hsc_home_unit_maybe hsc_env)
      mod_nm
...
lookupExtraDefs = do
  ...
  liftModule ←
    lookupModule (mkModuleName "InversionOfControl.Lift") (fsLit "inversion-of-control")
  typeDictModule ←
  ...
  getFamTC ← tcLookupTyCon =<< lookupOrig typeDictModule (mkTcOcc "GetFam")
...
solve Opts ExtraDefs{..} evBindsVar givens wanteds = do
  ...
  let substitute ∷ FollowerUFM → FollowerUFM → Type → TcPluginM Substitution
          ...
          Just (tycon, args)
            | tycon == toConstraintTC → do
                ...
                case values of
                  ...
                  Nothing → stucked
            | tycon == getTC || tycon == getFamTC → do
                handleGetTC tycon traverseUFM followerUFM args >>= \case
      ...
      handleGetTC myGetTC traverseUFM followerUFM args = do
        -- tcPluginIO $ putStrLn $ showPprUnsafe (ppr args)
        let (keyT : dictT : otherArgs) = args
        let stucked = unchanged (TyConApp myGetTC args)
        let goWithKey key = do
              ...
              extractDictUntilKey key dict dictTSub False <&> \case
                Left dictTSub' → do
                  ...
                  Left
                    Substitution
                      { sub_result = TyConApp myGetTC (keyT : sub_result dictTSub' : map sub_result otherArgs')
      ...
      handleLiftsUntilTC followerUFM args = do
        let [keyT, dictT] = args
        let stucked = unchanged (TyConApp liftsUntilTC args)
        let goWithKey key = do
      ...
      countLiftsUntilKey
        ∷ FastString → LiftDict' → Substitution → Bool → TcPluginM (Either (Int, Substitution) Int)
                  ...
                  (tycon, dargs)
                    ...
                    | tycon == followTC → Just do
                        ...
                        let dict2Sub =
                              dict2Sub1
                                { sub_changeFree =
                                    sub_changeFree dict2Sub1 && sub_changeFree dict2Sub0
                                }
                        let followDict2SubR =
                              TyConApp followTC $ prependKind $ sub_result dict2Sub
                        let sub2 =
                              Substitution
                                { sub_changeFree =
                                    sub_changeFree sub && sub_changeFree dict2Sub && not followed
                    ...
                    | tycon == getTC || tycon == getFamTC → Just do
                        handleGetTC tycon ld_env ld_env dargs >>= \case
                          Right (key, dict', []) → do
                            finishDictElem key dict'
      ...
      extractDictUntilKey
        ∷ FastString → CachedDict' → Substitution → Bool → TcPluginM (Either Substitution CachedDict')
                  ...
                  myTryFollow rest $
                    splitTyConApp_maybe >=> \case
                      (tycon, dargs)
                        ...
                        | tycon == getTC || tycon == getFamTC → Just do
                            handleGetTC tycon cd_env cd_env dargs >>= \case
                              Right (key, dict', []) → do
                                finishDictElem key dict'
                                dict'' ← getFinishedValue key dict'
