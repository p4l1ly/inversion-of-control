...
data ExtraDefs = ExtraDefs
  ...
  , typedictTyCon :: TyCon
  , substituedCts :: IORef (UniqFM EvVar Ct)
  }
...
lookupExtraDefs = do
  ...
  substituedCts <- tcPluginIO $ newIORef emptyUFM
  return ExtraDefs{..}
...
solve Opts{..} ExtraDefs{..} evBindsVar givens wanteds = do
  ...
  let reduceCtPred pred = do
        case classifyPredType pred of
          ...
          _ -> reduce False pred
  let reduceCt ct = do
        pastSubs <- tcPluginIO $ readIORef substituedCts
        case lookupUFM pastSubs (ctEvId ct) of
          Just ct' -> return $ Left ct'
          Nothing ->
            reduceCtPred (ctPred ct) >>= \case
              Just pred' -> return $ Right $ Just pred'
  ...
  newGivens <- for (zip givens' givens) \case
    (Left ct', _) -> return $ Left ct'
    (Right Nothing, _) -> return $ Right Nothing
    (Right (Just pred), ct) -> do
      let EvExpr ev = evCast (ctEvExpr $ ctEvidence ct) $ pluginCo (ctPred ct) pred
      ct' <- mkNonCanonical <$> newGiven evBindsVar (ctLoc ct) pred ev
      pastSubs <- tcPluginIO $ readIORef substituedCts
      tcPluginIO $ writeIORef substituedCts (addToUFM pastSubs (ctEvId ct) ct')
      return $ Right $ Just ct'
  newWanteds <- for (zip wanteds' wanteds) \case
    (Left ct', _) -> return $ Left ct'
    (Right Nothing, _) -> return $ Right Nothing
    (Right (Just pred), ct) -> do
      ct' <- mkNonCanonical <$> GHC.Tc.Plugin.newWanted (ctLoc ct) pred
      pastSubs <- tcPluginIO $ readIORef substituedCts
      tcPluginIO $ writeIORef substituedCts (addToUFM pastSubs (ctEvId ct) ct')
      return $ Right $ Just ct'

  let substEvidence ct ct' = evCast (ctEvExpr $ ctEvidence ct') $ pluginCo (ctPred ct') (ctPred ct)
  let removedGivens = [] -- [(substEvidence ct ct', ct) | (Left ct', ct) <- zip newGivens givens]
  let removedWanteds = [(substEvidence ct ct', ct) | (Left ct', ct) <- zip newWanteds wanteds]
  tcPluginTrace "---Plugin solve---" $ ppr givens $$ ppr wanteds
  tcPluginTrace "---Plugin newGivens---" $ ppr newGivens
  tcPluginTrace "---Plugin newWanteds---" $ ppr newWanteds
  ...
  return $
    TcPluginOk
      ...
      (catMaybes $ map (either (const Nothing) id) $ newGivens ++ newWanteds)
